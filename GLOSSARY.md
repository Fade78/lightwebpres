...
| **theme selector** | A runtime selection token from the effective catalogue: a bare slug, qualified native id such as `builtin:light`, `all`, `essential`, or `X:Y` such as `bg:light`, `fam:terrain` or `bgh:red`; several tokens add their matches. `builtin:<slug>` forces the built-in entry when a local slug shadows it. |
...
| **effective catalogue** | The canonical theme resources visible to one operation: the qualified native `builtin:light` resource plus palette entries merged from embedded, installed, user and, for a series operation, `templates/themes/` layers. A higher layer replaces a colliding bare slug's whole entry; a local bare `light` does not replace `builtin:light`. |
...
| **native resource** | Built-in reusable layout `builtin:standard` or minimal theme `builtin:light`. The native preset selector is `builtin/standard`. `builtin:light` is present in the global theme index, but its symbolic property layer remains owned by the native Identity Kit. Using a native layout inside a kit preserves that kit's chrome. |
...
| **resource origin** | Loading scope computed by loaders: built-in, installed, user or series-local. The picker displays both raw origins `embedded` and `builtin` as **Built-in** (French **Intégré**), without changing their payload values. Origin describes where a resource was found, not its collection, identity or palette `source` attribution, and is never declared in a manifest. Kits carry no extension, inter-kit dependency, provenance, filiation or authenticity record. |
